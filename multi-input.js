const inputTag = document.getElementById("inputTag").children;
...
function interviewerRemove(val){
  ...
  L2 = [];
  L3 = [];
  let newInterviwer = multiInput.getValues();
  
  const filterData = tempArr.filter((ele)=>{
    return ele.name !== val 
  })

  for(let i = 0; i < filterData.length; i++){
    switch(filterData[i].level){
      case "L1":
          L1.push(filterData[i]);
          // console.log("L1",L1);
          break;
      case "L2":
          L2.push(filterData[i]);
          // console.log("L2",L2);
          break;
      case "L3":
          L3.push(filterData[i]);
         // console.log("L3",L3);
          break;
    }
  }
  ...
  tempArr = filterData;
  for(let i = 0; i < newInterviwer.length; i++){
    tempArr[i].index = i.toString();
  }

  res = resultArr(tempArr);
  displingInterviewerWithLevel(res);
}
...
function fun(level){
  ...
  //  values.textContent = `Got ${multiInput.getValues().join(',')}!`;
    ...
    console.log("tempArr... this one",tempArr)
...
function filterMyData(arr) {
  console.log('working filter data');
...
function findClass(node){
  ...
  for (let index = 0; index < node.length; index++) {
    ...
    //finding elements whose class is 'item'
...
function displingInterviewerWithLevel(arr){
